...
class aPILOTO(ctk.CTkToplevel):
    ...
    def add_row(self):
        ...
        label = ctk.CTkLabel(new_row,font=font1, text=f"Producto {len(self.entries) + 1}:",text_color='#000',bg_color='#D0FCF6')
    ...
    def __init__(self, *args, **kwargs):
        ...
        def add_to_treeview():
            try:
                with mysql.connector.connect(host='localhost', user='root', password='admin', db='Jardineria') as conexion:
                    with conexion.cursor() as cursor:
                        consulta = "SELECT tp.nombre AS nombre_producto, m.razon, m.cantidad, u.nombre AS nombre_ubicacion, m.fecha FROM movimiento m JOIN tipo_producto tp ON m.sku = tp.sku JOIN ubicaciones u ON m.ubicacion = u.cod_ubi"
                        cursor.execute(consulta)
                        resultado = cursor.fetchall()
        ...
        self.save_button = ctk.CTkButton(self, text="Guardar Datos",font=font1,command = lambda:[self.save_data(),add_to_treeview()],border_color='#68462b',border_width=3,text_color='#fff',fg_color='#8E6537',hover_color='#78552D',bg_color='#91a398',cursor='hand2',corner_radius=15)
        self.save_button.place(relx=0.05, rely=0.90, anchor='nw')
